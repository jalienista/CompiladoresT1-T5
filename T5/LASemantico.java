...
public class LASemantico extends LAParserBaseVisitor<TabelaDeSimbolos.TipoLA>{
    ...
    // Adição para T5: permite acesso aos escopos pelo gerador de código
    ...
    */
    ...
    private List<TabelaDeSimbolos.ParametroInfo> coletarParametros(LAParser.ParametrosContext ctx){
        ...
        for (LAParser.ParametroContext param : ctx.parametro()){
            // Adição para T5: captura se o parâmetro é 'var'
            boolean isVar = param.VAR() != null;
            ...
            for (LAParser.IdentificadorContext id : param.identificador()){
                String nome = id.IDENT(0).getText();
                // Adição para T5: passa o flag isVar para o construtor
                lista.add(new TabelaDeSimbolos.ParametroInfo(nome, tipo, textoTipo, isVar));
            }
        }
        return lista;
    }
    ...
    public TabelaDeSimbolos.TipoLA visitCmdAtribuicao(LAParser.CmdAtribuicaoContext ctx){
        ...
        // Correção para T5: ao usar '^', o tipo é o tipo base do ponteiro, não PONTEIRO
        if (ctx.PONTEIRO() != null){
            if (resEsq.tipo == TabelaDeSimbolos.TipoLA.PONTEIRO && resEsq.nomeTipo != null){
                // O tipo base é obtido do nomeTipo do identificador
                tipoEsq = resolverTipo(resEsq.nomeTipo, idCtx.start, true);
                nomeTipoEsq = resEsq.nomeTipo;
            } else {
                // Caso o identificador não seja um ponteiro válido, reporta erro
                tipoEsq = TabelaDeSimbolos.TipoLA.INVALIDO;
                nomeTipoEsq = null;
                LASemanticoUtils.adicionarErroSemantico(idCtx.start,
                        "identificador " + nomeBase + " nao eh um ponteiro");
            }
        }
